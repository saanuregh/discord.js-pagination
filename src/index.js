...
const defaultCollectorEndHandler = async ({ paginatedEmbedMessage }) => {
	if (!paginatedEmbedMessage.deleted) {await paginatedEmbedMessage.reactions.removeAll();}
};
...
const paginationEmbed = async (receivedMessage, pages,
	...
	reactionCollector.on('collect', async (reaction, user) => {
		...
		try {
			...
			if (!paginatedEmbedMessage.deleted && currentPage != currentPageIndex && currentPageIndex >= 0 && currentPageIndex < pages.length) {await paginatedEmbedMessage.edit({ embeds: [pages[currentPageIndex].setFooter(footerResolver(currentPageIndex, pages.length))] });}
		} catch(error) {
